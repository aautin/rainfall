...
```
The base pointer is pushed on the stack, then the stack is 16-byte aligned (16 - 4 = 12 bytes of padding)
...
```
 ...
 8048495:	c9                   	leave
```
Gets is called (which is unsafe since it doesn't check the size of the input), if we fill the buffer (64 bytes) + fill the aligment (12 bytes) + fill the pushed ebp (4 bytes) we can THEN write the address it will return
